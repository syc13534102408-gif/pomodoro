...
- **三种模式计时**：专注 / 短休息 / 长休息，每第 4 次完成自动进长休息；
  计划时长到点**不自动结束**，继续累计 `+MM:SS`，由用户点「完成」才落记录；
  计时中切换模式**不打断**——自动挂起，切回即从冻结点继续
- **番茄以时长计**：每满 50 分钟 1 个番茄（缺口不足 15 分钟补齐），统计日以**凌晨 3 点**为分界
...
- **会话实时同步**（安卓 ↔ macOS，可选开关）：一边开始专注，另一边约 3 秒内同步开始；暂停/完成对等互相同步
